...
```{r}
network <- (model2network("[HD][IJ|HD][EW][MAR][TID|EW:MAR][FC][DMK][DMNT|FC:DMK][FL|IJ:TID:DMNT]"))
...
```{r}
jDMNT <- setEvidence(junction, nodes = "DMNT", states = "Low")
```
...
```{r}
#High
jDMK <- setEvidence(junction, nodes = "DMK", states = "High")
